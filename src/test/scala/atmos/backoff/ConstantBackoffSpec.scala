...
class ConstantBackoffSpec extends FlatSpec with Matchers {
  ...
  "ConstantBackoff" should "produce the same backoff on any combination of inputs" in {
    for {
      backoff <- 1L to 100L map (100.millis * _)
      policy = ConstantBackoff(backoff)
      outcome <- Seq(Success(result), Failure(thrown))
      attempt <- 1 to 10
    } policy.nextBackoff(attempt, outcome) shouldEqual backoff
  }
