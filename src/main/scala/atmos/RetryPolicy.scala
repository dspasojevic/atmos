...
case class RetryPolicy(
  ...
  private abstract class RetryOperation(name: Option[String])(implicit val clock: Clock) {
    ...
    protected def afterAttempt[T](outcome: Try[T]): Outcome[T] = outcome match {
      case Success(result) =>
        results.applyOrElse(result, ResultClassification) match {
  ...
  /**
    ...
    */
  private final class SyncRetryOperation[T](name: Option[String], operation: () => T)(implicit c: Clock) extends RetryOperation(name) {
    ...
    def run(): T =
      afterAttempt {
        ...
        catch { case thrown: Throwable => Failure(thrown) }
      } match {
  ...
  /**
    ...
    */
  private final class AsyncRetryOperation[T](name: Option[String], operation: () => Future[T])(implicit ec: ExecutionContext, c: Clock) extends RetryOperation(name) with (Try[T] => Unit) {
    ...
    def run(): Future[T] = {
      try spawn() onComplete this
      catch { case thrown: Throwable => promise.failure(thrown) }
      promise.future
    }
    ...
    private def spawn(): Future[T] =
      try operation()
      catch { case thrown: Throwable => Future.failed(thrown) }
